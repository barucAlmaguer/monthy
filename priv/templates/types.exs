defmodule ValiotAppWeb.Schema.Types do
  ...
  object <%= inspect schema |> Inflex.underscore |> String.to_atom %> do
    ...
    <% :has_one -> %>field(<%= inspect type |> Inflex.underscore |> String.to_atom %>, <%= inspect type |> Inflex.underscore |> String.to_atom %>, resolve: assoc(<%= inspect type |> Inflex.underscore |> String.to_atom %>))
