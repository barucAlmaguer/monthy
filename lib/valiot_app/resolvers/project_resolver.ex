defmodule ValiotApp.ProjectResolver do
  ...
  def all(item, args, %{context: %{current_token: current_token}}) do
    case AuthHelper.authorized?(:read, :project, Map.get(current_token, :id)) do
  ...
  def create(args, %{context: %{current_token: current_token}}) do
    case AuthHelper.authorized?(:create, :project, Map.get(current_token, :id)) do
      true ->
  ...
  def update(%{id: id, project: project_params}, %{context: %{current_token: current_token}}) do
    case AuthHelper.authorized?(:update, :project, Map.get(current_token, :id)) do
      true ->
        case Api.get_project(id) do
          nil -> {:error, "Resource with id #{id} was not found"}
  ...
  def delete(%{id: id}, %{context: %{current_token: current_token}}) do
    case AuthHelper.authorized?(:delete, :project, Map.get(current_token, :id)) do
      true ->
        case Api.get_project(id) do
          nil -> {:error, "Resource with id #{id} was not found"}
