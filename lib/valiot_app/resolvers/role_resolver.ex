defmodule ValiotApp.RoleResolver do
  ...
  def all(item, args, %{context: %{current_token: current_token}}) do
    case AuthHelper.authorized?(:read, :role, Map.get(current_token, :id)) do
  ...
  def create(args, %{context: %{current_token: current_token}}) do
    case AuthHelper.authorized?(:create, :role, Map.get(current_token, :id)) do
      true ->
  ...
  def update(%{id: id, role: role_params}, %{context: %{current_token: current_token}}) do
    case AuthHelper.authorized?(:update, :role, Map.get(current_token, :id)) do
      true ->
        case Api.get_role(id) do
          nil -> {:error, "Resource with id #{id} was not found"}
  ...
  def delete(%{id: id}, %{context: %{current_token: current_token}}) do
    case AuthHelper.authorized?(:delete, :role, Map.get(current_token, :id)) do
      true ->
        case Api.get_role(id) do
          nil -> {:error, "Resource with id #{id} was not found"}
