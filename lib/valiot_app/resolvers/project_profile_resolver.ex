defmodule ValiotApp.ProjectProfileResolver do
  ...
  def all(item, args, %{context: %{current_user: current_user}}) do
    case AuthHelper.authorized?(:read, :project_profile, Map.get(current_user, :id)) do
  ...
  def create(args, %{context: %{current_user: current_user}}) do
    case AuthHelper.authorized?(:create, :project_profile, Map.get(current_user, :id)) do
      true ->
  ...
  def update(%{id: id, project_profile: project_profile_params}, %{context: %{current_user: current_user}}) do
    case AuthHelper.authorized?(:update, :project_profile, Map.get(current_user, :id)) do
      true ->
        case Api.get_project_profile(id) do
          nil -> {:error, "Resource with id #{id} was not found"}
  ...
  def delete(%{id: id}, %{context: %{current_user: current_user}}) do
    case AuthHelper.authorized?(:delete, :project_profile, Map.get(current_user, :id)) do
      true ->
        case Api.get_project_profile(id) do
          nil -> {:error, "Resource with id #{id} was not found"}
