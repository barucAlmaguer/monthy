defmodule ValiotApp.IssueResolver do
  ...
  def all(item, args, %{context: %{current_token: current_token}}) do
    case AuthHelper.authorized?(:read, :issue, Map.get(current_token, :id)) do
  ...
  def create(args, %{context: %{current_token: current_token}}) do
    case AuthHelper.authorized?(:create, :issue, Map.get(current_token, :id)) do
      true ->
  ...
  def update(%{id: id, issue: issue_params}, %{context: %{current_token: current_token}}) do
    case AuthHelper.authorized?(:update, :issue, Map.get(current_token, :id)) do
      true ->
        case Api.get_issue(id) do
          nil -> {:error, "Resource with id #{id} was not found"}
  ...
  def delete(%{id: id}, %{context: %{current_token: current_token}}) do
    case AuthHelper.authorized?(:delete, :issue, Map.get(current_token, :id)) do
      true ->
        case Api.get_issue(id) do
          nil -> {:error, "Resource with id #{id} was not found"}
